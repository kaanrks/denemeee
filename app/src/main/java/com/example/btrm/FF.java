...
public class FF extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Ref = FirebaseDatabase.getInstance().getReferenceFromUrl("https://root-124bb-default-rtdb.firebaseio.com");
    }
            ...
            newFeedbackRef.setValue(feedbackData).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(FF.this, "Feedback sent successfully!", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(FF.this, "Failed to send feedback: " + task.getException().getMessage(), Toast.LENGTH_LONG).show();
                    }
                }
            });
        } else {
